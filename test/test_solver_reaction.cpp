...
int main() {
    std::cout << "===========================================\n";
    std::cout << "  Schwarz Solver Test: strong reaction\n";
    std::cout << "===========================================\n";
    ...
    pde_params.mu = 1.0;
    pde_params.c = 10.0; // strong reaction term
    pde_params.omega = omega;
    pde_params.dirichlet = dirichlet_bcs;
    pde_params.f = [](Real x) {
        return (M_PI * M_PI + 10.0) * std::sin(M_PI * x); // -u'' + 10 u = (pi^2+10) sin(pi x)
    };
    ...
    if (l2_error < 1e-3) {
        std::cout << "[PASSED] Reaction test within tolerance." << std::endl;
        return 0;
    }
